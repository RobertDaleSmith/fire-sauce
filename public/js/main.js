	...
	$('#titlebar_wrapper .btn').bind('click', function(e) {
		...
		if(!$(this).hasClass('active')){
			...
			if(isFor=="history_wrapper"){
				renderHistory(hist.channels);
			}else if(isFor=="watch_wrapper"){
				scrollToTrack();
			}else if(isFor=="channels_wrapper"){
				scrollToTrack();
			}
...
function checkExtCompatibility() {
...
function updateChannelInfo(user){
	...
	$('#history_wrapper .channel').removeClass('active');
...
function renderTracks(tweets){
	...
	$.each( tweets, function( idx, tweet ) {
		...
		var element = $('<li/>')
			...
			.append($('<img>')
				.addClass("avatar")
				.attr('src', hist.channels[hist.watching].info.profile_image_url)
			)
			.append($('<div/>')
				.addClass("user_name")
				.text( hist.channels[hist.watching].info.name )
			)
			.append($('<div/>')
				.addClass("user_screen_name")
				.text( "@"+hist.channels[hist.watching].info.screen_name )
			)
			.append($('<a/>')
				.addClass("started")
...
function renderHistory(channels){
	...
	$("#watched_wrapper").html('').css('display','');
	...
	if(watchedCount<1)$("#watched_wrapper").css('display','none');

}

function renderChannels(channels){

	$("#top100_wrapper").html('').css('display','');
	...
	$.each( sortable, function( idx, channel ) {
		...
		$("#top100_wrapper").prepend(element);
		
		
		
	});

	if(suggestCount<1)$("#top100_wrapper").css('display','none');
